...
with st.expander("📝 새로운 건강 기록 입력 (클릭)", expanded=True):
    with st.form("health_form", clear_on_submit=True):
        ...
        with c2: 
            d = st.date_input("측정 날짜", now_display.date())
        ...
        note = st.text_area("🗒️ 특이사항 (증상, 컨디션 등)")

        # --- 교차 복용 체크 로직 ---
        child_history = st.session_state.df[st.session_state.df['이름'] == name]
        if not child_history.empty and med in ["맥시부펜", "세토펜"]:
            # 마지막 '약' 기록 찾기 (선택 안 함 제외)
            med_history = child_history[child_history['약 종류'] != "선택 안 함"]
            if not med_history.empty:
                last_med = med_history.iloc[-1]['약 종류']
                if last_med == med:
                    st.warning(f"⚠️ 주의: {name}가 마지막으로 복용한 약도 **{last_med}**입니다. 교차 복용이 필요한지 확인하세요!")
        
        st.info("💡 저장 버튼을 누르는 시점의 실시간 시간이 자동으로 기록됩니다.")
        
        if st.form_submit_button("💾 실시간 시간으로 기록 저장", use_container_width=True):
            now_realtime = datetime.datetime.now()
            f_date = d.strftime("%y.%m.%d")
            ampm = "오전" if now_realtime.hour < 12 else "오후"
            h12 = 12 if now_realtime.hour % 12 == 0 else now_realtime.hour % 12
            f_time = f"{ampm} {h12}:{now_realtime.strftime('%M')}"
...
for i, c_name in enumerate(child_names):
    with g_cols[i]:
        ...
        if not f_df.empty:
            ...
            fig.update_layout(
                height=220, margin=dict(l=5, r=5, t=25, b=5),
                paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                showlegend=False,
                xaxis=dict(showgrid=False, zeroline=False, color='white', tickfont=dict(size=9)),
